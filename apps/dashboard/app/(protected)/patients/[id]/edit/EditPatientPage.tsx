...
type FormData = {
  name: string; age: number; gender: string; admission: string
...
export default function EditPatientPage({ patient }: { patient: Patient }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/patients/${patient.id}`, {
        method: 'PATCH',
      ...
      if (!res.ok) {
        ...
        setSaveError(body?.detail ?? `Failed to save (${res.status}).`)
        setSaving(false)
        return
      }
  ...
  if (step === 'calculating') return <LoadingOverlay title="Recalculating risk…" sub="Analysing updated clinical factors" />

  if (step === 'result' && result) {
    const tv = toneVar(result.tone)
    return (
      <Screen bg="var(--bg)" topBar={<TopBar title="Updated assessment"
    ...
    <Screen bg="var(--bg)" topBar={
      ...
      bottomNav={
        <div style={{ padding: '14px var(--pad) 20px', background: 'var(--surface)', borderTop: '1px solid var(--line)' }}>
          <Button full size="lg" iconRight="arrowRight" onClick={submit}>Recalculate delirium risk</Button>
        </div>
      }>
